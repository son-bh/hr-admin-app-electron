...
import { RequestOutCategoryOption, StatusOption } from '@/shared/constants';
...
export default function Filter({ handleFilterChange }: IFilterProps) {
  ...
  return (
    <div className='mb-5 grid 2xl:grid-cols-7 xl:grid-cols-5 sm:grid-cols-3 grid-cols-1 sm:gap-4 gap-2'>
      <div>
        <Label>Từ khoá</Label>
        <Input
          key={`search-${resetKey}`}
      ...
      <div>
        ...
        <RSelect
          ...
          value={filter.type}
          options={RequestOutCategoryOption}
          label='loại nghỉ '
          onChange={newValue => {
            setFilter(prev => ({
              ...prev,
              type: newValue as unknown as IOptionSelect,
            }));
          }}
      ...
      <div className='flex items-end gap-2'>
        <Button size='sm' className='h-11' onClick={handleReset}>
          Cài lại
        </Button>
        <Button
          size='sm'
