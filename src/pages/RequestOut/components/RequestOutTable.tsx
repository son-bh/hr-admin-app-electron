...
interface IUserListProps {
  requestData: Array<IRequestOut>;
  handleAction: (data: IRequestOut, modalType: string) => void;
}

export default function RequestOutTable({
  requestData,
  handleAction,
}: IUserListProps) {
  const [sorting, setSorting] = useState<SortingState>([]);
  const columnHelper = createColumnHelper<IRequestOut>();
  const userInfo = useUserStore(state => state.userInfo);

  const columns = useMemo<ColumnDef<IRequestOut, any>[]>(
    () => [
      ...
      columnHelper.accessor('type', {
        ...
        cell: info => (
          <div>
            {RequestOutCategoryLabel[
              info.getValue() as keyof typeof RequestOutCategoryLabel
            ] || '--'}
          </div>
        ),
      }),
      ...
      columnHelper.accessor('from', {
        header: () => 'Giờ dự tính chấm công',
        cell: info => <div>{formatDate(info.getValue(), 'HH:mm')}</div>,
      }),
